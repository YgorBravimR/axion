...
describe("accumulateIrrf", () => {
	it("positive days only — IRRF 1% on each positive day gross", () => {
		...
		const result = accumulateIrrf(days, asBasisPoints(100)) // 100bps = 1%
	...
	it("negative day → no IRRF withheld", () => {
		const days = [{ date: new Date("2026-01-02"), grossPnlCents: -30000 }]
		const result = accumulateIrrf(days, asBasisPoints(100))
	...
	it("empty days array → total 0, empty byDay", () => {
		const result = accumulateIrrf([], asBasisPoints(100))
		expect(result.totalIrrfCents).toBe(0)
