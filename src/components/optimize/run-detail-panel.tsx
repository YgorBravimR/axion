	...
	return (
		<div className="border-bg-300 bg-bg-200 space-y-m-400 p-m-400 rounded-lg border">
			<div className="flex items-center justify-between">
				<h3 className="text-h3 text-txt-100 font-semibold">{run.label}</h3>
			...
			<div className="bg-bg-100/50 gap-s-300 p-s-300 grid grid-cols-2 rounded-lg sm:grid-cols-4">
				<ConfigItem label={t("configStop")} value={formatStopConfig(run, t)} />
				<ConfigItem
					label={t("configTarget")}
					value={formatTargetConfig(run, t)}
				/>
				<ConfigItem
					label={t("configSizing")}
					value={formatSizingConfig(run, t)}
				/>
				<ConfigItem
					label={t("configSlippage")}
					value={`${run.recipe.slippageTicks} ticks`}
				/>
			</div>
...
const formatStopConfig = (run: OptimizationRun, t: TranslateFn): string => {
	...
	switch (stop.type) {
		case "pct_range":
			return t("runDetail.stopPctRange", { pct: stop.pct })
		case "fixed_points":
			return stop.points === 0
				? t("runDetail.stopPivotRef")
				: t("runDetail.stopFixedPts", { points: stop.points })
		case "full_range":
			return t("runDetail.stopFullRange", { buffer: stop.ticksBuffer })
	}
}

const formatTargetConfig = (run: OptimizationRun, t: TranslateFn): string => {
	if (run.recipe.target.type !== "fixed_levels") {
		return "—"
	}
	...
	return levels
		.map(
			(l) =>
				`${l.value}${l.mode === "r_multiple" ? t("runDetail.unitR") : t("runDetail.unitPts")}`
		)
		.join(" / ")
}
...
const formatSizingConfig = (run: OptimizationRun, t: TranslateFn): string => {
	...
	if (sizing.type === "fixed_lots") {
		return t("runDetail.sizingLots", { lots: sizing.lots })
	}
	return t("runDetail.sizingRisk", {
