...
const RunsComparisonTable = ({
	...
	const bestRunId = useMemo(() => {
		const viable = runs.filter((r) => r.summary.totalTrades >= MIN_TRADES_THRESHOLD)
	...
	const columns = useMemo<ColumnDef<OptimizationRun, unknown>[]>(() => [
		{
			...
			cell: ({ row }) => {
				...
				return (
					...
					>
						{run.pinned
							? <Pin className="h-3.5 w-3.5 text-acc-100" />
							: <PinOff className="h-3.5 w-3.5 text-txt-300" />
						}
					</Button>
				)
			},
			...
			cell: ({ row }) => {
				...
				return (
					<div className="flex items-center gap-s-200">
						...
						{isBest && (
							<Badge id={`best-${run.id}`} variant="outline" className="border-acc-100 text-acc-100 text-micro px-s-100 py-0">
								{t("bestRun")}
							</Badge>
						)}
						{run.summary.totalTrades < MIN_TRADES_THRESHOLD && (
							<AlertTriangle className="h-3 w-3 text-acc-100 shrink-0" aria-label={t("lowTradeWarning")} />
						)}
					</div>
				)
			},
		...
		},
		{
			...
			header: t("trades"),
			cell: ({ getValue }) => (
				<span className="text-small text-txt-100 tabular-nums">{getValue<number>()}</span>
			),
		},
		{
			...
			cell: ({ getValue }) => {
				...
				const color = value >= 50 ? "text-fb-success" : value < 40 ? "text-fb-error" : "text-txt-100"
				return <span className={`text-small tabular-nums ${color}`}>{value.toFixed(1)}%</span>
			},
		},
		{
			...
			cell: ({ getValue }) => {
				const value = getValue<number>()
				const color = value >= 1.5 ? "text-fb-success" : value < 1 ? "text-fb-error" : "text-txt-100"
				return <span className={`text-small font-medium tabular-nums ${color}`}>{value.toFixed(2)}</span>
			},
		},
		{
			...
			cell: ({ getValue }) => {
				...
				const color = cents > 0 ? "text-fb-success" : cents < 0 ? "text-fb-error" : "text-txt-100"
				return <span className={`text-small tabular-nums ${color}`}>{formatCentsAsCurrency(cents, "BRL")}</span>
			},
		},
		{
		...
		},
		{
			...
			cell: ({ getValue }) => {
				...
				return <span className={`text-small tabular-nums ${color}`}>{value.toFixed(2)}</span>
			},
		},
		{
			...
			cell: ({ getValue }) => {
				const value = getValue<number>()
				const color = value > 0 ? "text-fb-success" : value < 0 ? "text-fb-error" : "text-txt-100"
				return <span className={`text-small tabular-nums ${color}`}>{value.toFixed(2)}R</span>
			},
		},
		{
			id: "actions",
			...
			cell: ({ row }) => {
				...
				return (
					<div className="flex items-center gap-s-100">
						...
						>
							<ChevronRight className={`h-3.5 w-3.5 text-txt-300 transition-transform ${isExpanded ? "rotate-90" : ""}`} />
						</Button>
						<Button
							id={`delete-${run.id}`}
							variant="ghost"
