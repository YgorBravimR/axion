...
const DezkEntrySection = memo(({ recipe, onRecipeChange }: DezkEntrySectionProps) => {
	...
	if (recipe.entry.type !== "macd_wma_alignment") return null
	const config = recipe.entry.config as MACDWMAConfig

	const update = (field: string, value: number | boolean) => {
		onRecipeChange({
			...recipe,
			entry: { type: "macd_wma_alignment", config: { ...config, [field]: value } },
		})
	}
	...
	return (
		<div className="border-bg-300 bg-bg-200 space-y-m-400 rounded-lg border p-m-400">
			...
			<div className="space-y-s-200">
				<p className="text-small font-medium text-txt-200">MACD</p>
				<div className="gap-m-400 grid grid-cols-3">
					<div className="space-y-s-200">
						<Label id="label-macd-fast">{t("macdFast")}</Label>
						<Input id="macd-fast" type="number" value={config.macdFast}
							onChange={(e) => update("macdFast", parseInt(e.target.value) || 12)} />
					</div>
					<div className="space-y-s-200">
						<Label id="label-macd-slow">{t("macdSlow")}</Label>
						<Input id="macd-slow" type="number" value={config.macdSlow}
							onChange={(e) => update("macdSlow", parseInt(e.target.value) || 26)} />
					</div>
					<div className="space-y-s-200">
						<Label id="label-macd-signal">{t("macdSignal")}</Label>
						<Input id="macd-signal" type="number" value={config.macdSignal}
							onChange={(e) => update("macdSignal", parseInt(e.target.value) || 15)} />
					</div>
				</div>
			</div>
			...
			<div className="gap-m-400 grid grid-cols-2 sm:grid-cols-4">
				<div className="space-y-s-200">
					...
					<Input id="wma-fast" type="number" value={config.wmaFast}
						onChange={(e) => update("wmaFast", parseInt(e.target.value) || 9)} />
				</div>
				<div className="space-y-s-200">
					<Label id="label-wma-slow">{t("wmaSlow")}</Label>
					<Input id="wma-slow" type="number" value={config.wmaSlow}
						onChange={(e) => update("wmaSlow", parseInt(e.target.value) || 21)} />
				</div>
				<div className="space-y-s-200">
					<Label id="label-candles-after">{t("candlesAfter")}</Label>
					<Input id="candles-after" type="number" value={config.candlesAfterAlignment}
						onChange={(e) => update("candlesAfterAlignment", parseInt(e.target.value) || 2)} />
				</div>
				<div className="space-y-s-200">
					<Label id="label-stop-buffer">{t("stopBuffer")}</Label>
					<div className="flex items-center gap-s-100">
						<Input id="stop-buffer" type="number" value={config.stopBufferPoints}
							onChange={(e) => update("stopBufferPoints", parseInt(e.target.value) || 30)} />
