	...
	({ recipe, onRecipeChange }: StopProtectionSectionProps) => {
		...
		const initialStopOptions = useMemo(
			() => [
				{
					value: "pct_range",
					label: t("stopPctRange"),
					description: t("stopPctRangeDesc"),
				},
				{
					value: "fixed_points",
					label: t("stopFixedPoints"),
					description: t("stopFixedPointsDesc"),
				},
				{
					value: "full_range",
					label: t("stopFullRange"),
					description: t("stopFullRangeDesc"),
				},
			],
			[t]
		)
		...
		const handleInitialStopValue = (value: number) => {
			if (initialType === "pct_range") {
				onRecipeChange({
					...recipe,
					stop: { ...stopConfig, initial: { type: "pct_range", pct: value } },
				})
				...
				onRecipeChange({
					...
					stop: {
						...stopConfig,
						initial: { type: "fixed_points", points: value },
					},
				})
				...
				onRecipeChange({
					...
					stop: {
						...stopConfig,
						initial: { type: "full_range", ticksBuffer: value },
					},
				})
			}
		}
		...
		const handleBreakevenToggle = (enabled: boolean) => {
			if (!enabled) {
				onRecipeChange({
					...recipe,
					stop: { ...stopConfig, breakeven: undefined },
				})
				return
			}
			...
			() => [
				{
					value: "on_partial",
					label: t("beOnPartial"),
					description: t("beOnPartialDesc"),
				},
				{
					value: "on_pct_risk",
					label: t("beOnPctRisk"),
					description: t("beOnPctRiskDesc"),
				},
			],
			[t]
		)
		...
		const handleBreakevenTypeChange = (type: string) => {
			if (type === "on_partial") {
				onRecipeChange({
					...recipe,
					stop: { ...stopConfig, breakeven: { type: "on_partial" } },
				})
		...
		const handleTrailingToggle = (enabled: boolean) => {
			if (!enabled) {
				onRecipeChange({
					...recipe,
					stop: { ...stopConfig, trailing: undefined },
				})
				return
			}
			onRecipeChange({
				...
				stop: {
					...stopConfig,
					trailing: { type: "price_distance", distance: 100 },
				},
			})
		}
		...
		return (
			<div className="border-bg-300 bg-bg-200 space-y-m-500 p-m-400 rounded-lg border">
				...
				{/* Additive plugins */}
				<div className="space-y-s-300">
					<p className="text-small text-txt-200 font-medium">{t("addOns")}</p>
						...
						>
							<div className="space-y-s-200">
								...
								<Input
									...
									type="number"
									value={
										stopConfig.trailing?.type === "price_distance"
											? stopConfig.trailing.distance
											: 100
									}
									onChange={(e) => {
										...
										onRecipeChange({
											...
											stop: {
												...stopConfig,
												trailing: { type: "price_distance", distance },
											},
										})
									}}
								/>
