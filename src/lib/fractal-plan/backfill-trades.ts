...
const backfillTradesForAccount = async (input: BackfillInput): Promise<BackfillResult> => {
	const rows = await db
		.select({
			id: trades.id,
	...
	for (const row of rows) {
		const snapshot = await captureROnEntry({
			...
			entryDate: row.entryDate instanceof Date ? row.entryDate : new Date(row.entryDate as string),
		})
