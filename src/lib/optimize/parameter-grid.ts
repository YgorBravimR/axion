...
interface NumericSweepableParam {
	...
	dynamicDefaults?: (recipe: StrategyRecipe) => { min: number; max: number; step: number }
}
...
const applyStopType = (recipe: StrategyRecipe, type: string): StrategyRecipe => {
...
const applyTargetMode = (recipe: StrategyRecipe, mode: string): StrategyRecipe => {
	if (recipe.target.type !== "fixed_levels") return recipe
	return {
		...recipe,
		target: {
			...recipe.target,
			levels: recipe.target.levels.map((l) => ({ ...l, mode: mode as TargetMode })),
		},
	}
}

const applyTrailingType = (recipe: StrategyRecipe, type: string): StrategyRecipe => {
	const base = { ...recipe, stop: { ...recipe.stop } }
	switch (type) {
		case "indicator":
...
const getTargetUnitSuffix = (recipe: StrategyRecipe): string => {
	if (recipe.target.type !== "fixed_levels" || recipe.target.levels.length === 0) return ""
...
const getTargetDefaults = (recipe: StrategyRecipe): { min: number; max: number; step: number } => {
	if (recipe.target.type !== "fixed_levels" || recipe.target.levels.length === 0) {
		return { min: 1, max: 3, step: 0.5 }
	}
	const mode = recipe.target.levels[0].mode
	switch (mode) {
		case "r_multiple": return { min: 0.5, max: 3, step: 0.5 }
		case "pct_range": return { min: 500, max: 2000, step: 250 }
		case "pct_stop": return { min: 50, max: 200, step: 25 }
		case "fixed_points": return { min: 50, max: 500, step: 50 }
	}
}

const getTarget2Defaults = (recipe: StrategyRecipe): { min: number; max: number; step: number } => {
	if (recipe.target.type !== "fixed_levels" || recipe.target.levels.length === 0) {
		return { min: 1, max: 4, step: 0.5 }
	}
	const mode = recipe.target.levels[0].mode
	switch (mode) {
		case "r_multiple": return { min: 1, max: 4, step: 0.5 }
		case "pct_range": return { min: 1000, max: 3000, step: 500 }
		case "pct_stop": return { min: 100, max: 300, step: 50 }
		case "fixed_points": return { min: 100, max: 800, step: 100 }
	}
}
...
const ORB_PARAMS: SweepableParam[] = [
	// -- Enum: Stop Type --
	{
		...
		options: [
			{ value: "pct_range", labelKey: "stopType.pctRange", applyOption: (r) => applyStopType(r, "pct_range") },
			{ value: "full_range", labelKey: "stopType.fullRange", applyOption: (r) => applyStopType(r, "full_range") },
			{ value: "fixed_points", labelKey: "stopType.fixedPoints", applyOption: (r) => applyStopType(r, "fixed_points") },
		],
	},
	// -- Enum: Target Mode --
	{
		...
		condition: (r) => r.target.type === "fixed_levels" && r.target.levels.length > 0,
		getCurrentValue: (r) => r.target.type === "fixed_levels" ? r.target.levels[0].mode : "r_multiple",
		options: [
			{ value: "r_multiple", labelKey: "targetMode.rMultiple", applyOption: (r) => applyTargetMode(r, "r_multiple") },
			{ value: "pct_range", labelKey: "targetMode.pctRange", applyOption: (r) => applyTargetMode(r, "pct_range") },
			{ value: "pct_stop", labelKey: "targetMode.pctStop", applyOption: (r) => applyTargetMode(r, "pct_stop") },
			{ value: "fixed_points", labelKey: "targetMode.fixedPoints", applyOption: (r) => applyTargetMode(r, "fixed_points") },
		],
	},
	...
	},
	{
		...
		defaultMax: 4,
		defaultStep: 0.5,
		condition: (r) => r.target.type === "fixed_levels" && r.target.levels.length >= 2,
		unitSuffix: getTargetUnitSuffix,
...
const DEZK_PARAMS: SweepableParam[] = [
	// -- Enum: Target Mode --
	{
		...
		labelKey: "targetModeLabel",
		condition: (r) => r.target.type === "fixed_levels" && r.target.levels.length > 0,
		getCurrentValue: (r) => r.target.type === "fixed_levels" ? r.target.levels[0].mode : "fixed_points",
		options: [
			{ value: "r_multiple", labelKey: "targetMode.rMultiple", applyOption: (r) => applyTargetMode(r, "r_multiple") },
			{ value: "pct_range", labelKey: "targetMode.pctRange", applyOption: (r) => applyTargetMode(r, "pct_range") },
			{ value: "pct_stop", labelKey: "targetMode.pctStop", applyOption: (r) => applyTargetMode(r, "pct_stop") },
	...
	// -- Enum: Trailing Type --
	{
		...
		options: [
			{ value: "indicator", labelKey: "trailingType.indicator", applyOption: (r) => applyTrailingType(r, "indicator") },
			{ value: "price_distance", labelKey: "trailingType.priceDistance", applyOption: (r) => applyTrailingType(r, "price_distance") },
		],
	},
...
const getSweepableParams = (
	...
	const catalog = recipe.entry.type === "orb_breakout" ? ORB_PARAMS : DEZK_PARAMS

	return catalog.filter((param) => {
		if (!param.condition) return true

		// Base recipe condition check
		if (param.condition(recipe)) return true

		// For enum params, only the base check matters
		if (param.kind === "enum") return false
			...
			for (const [enumPath, values] of Object.entries(activeEnumValues)) {
				...
				for (const val of values) {
					...
					if (option) {
						...
						if (param.condition(variant)) return true
					}
				}
			}
		}
 ...
 */
const countCombinations = (ranges: ParameterRange[], baseRecipe: StrategyRecipe): number => {
	const enumRanges = ranges.filter((r): r is EnumParameterRange => r.kind === "enum")
	const numericRanges = ranges.filter((r): r is NumericParameterRange => r.kind === "numeric")

	if (enumRanges.length === 0 && numericRanges.length === 0) return 0

	const catalog = baseRecipe.entry.type === "orb_breakout" ? ORB_PARAMS : DEZK_PARAMS
	...
	const countNumericsForVariant = (variant: StrategyRecipe): number => {
		const applicable = numericRanges.filter((nr) => {
			const def = catalog.find((p) => p.kind === "numeric" && p.path === nr.path) as NumericSweepableParam | undefined
			return !def?.condition || def.condition(variant)
		})
		if (applicable.length === 0) return 1
		return applicable.reduce((acc, r) => acc * generateValues(r.min, r.max, r.step).length, 1)
	}

	// No enum ranges → simple numeric count against base recipe
	if (enumRanges.length === 0) {
		return numericRanges.reduce((acc, r) => acc * generateValues(r.min, r.max, r.step).length, 1)
	}
	...
	for (const combo of enumCombos) {
		...
		for (let i = 0; i < enumRanges.length; i++) {
			const option = enumRanges[i].enumDef.options.find((o) => o.value === combo[i])
			if (option) variant = option.applyOption(variant)
		}
...
const generateRecipeGrid = (
	...
	const enumValueArrays = enumRanges.map((er) => er.selectedValues)
	const enumCombos = enumValueArrays.length > 0
	...
	for (const combo of enumCombos) {
		...
		if (applicable.length === 0) {
			...
			const valueArrays = applicable.map((nr) => generateValues(nr.min, nr.max, nr.step))
