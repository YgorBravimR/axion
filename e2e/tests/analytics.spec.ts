...
test.describe("Analytics", () => {
	...
	test.describe("Page Layout", () => {
		test("should display page header", async ({ page, isMobile }) => {
			...
			const activeNav = page.locator(
				'a[aria-current="page"]:has-text("Analytics")'
			)
	...
	test.describe("Filter Panel", () => {
		test("should display filter panel", async ({ page }) => {
			const filterPanel = page.locator(
				'[data-testid="filter-panel"], .filter-panel, :has-text("Filter")'
			)
		...
		test("should have date range picker", async ({ page }) => {
			const dateFrom = page
				.locator('[name="dateFrom"], [data-testid="date-from"]')
				.or(page.getByLabel(/from|de/i))
			const dateTo = page
				.locator('[name="dateTo"], [data-testid="date-to"]')
				.or(page.getByLabel(/to|até/i))
		...
		test("should have asset filter", async ({ page }) => {
			const assetFilter = page
				.locator('[data-testid="asset-filter"], :has-text("Asset")')
				.first()
		...
		test("should have direction filter", async ({ page }) => {
			const directionFilter = page.locator(
				'[data-testid="direction-filter"], :has-text("Direction")'
			)
		...
		test("should have outcome filter", async ({ page }) => {
			const outcomeFilter = page.locator(
				'[data-testid="outcome-filter"], :has-text("Outcome")'
			)
		...
		test("should show active filter count", async ({ page }) => {
			...
			if (await assetFilter.isVisible()) {
				...
				const activeCount = page.locator(
					'[data-testid="active-filters"], .filter-count, :has-text("active")'
				)
	...
	test.describe("Variable Comparison Chart", () => {
		test("should display variable comparison section", async ({ page }) => {
			const section = page.locator(
				'[data-testid="variable-comparison"], :has-text("Performance")'
			)
			await expect(section.first()).toBeVisible()
		})

		test("should have group by selector", async ({ page }) => {
			const groupBy = page.locator(
		...
		test("should switch group by option", async ({ page }) => {
			const groupBySelect = page
				.locator('[data-testid="group-by"], select')
				.first()
	...
	test.describe("Cumulative P&L Chart", () => {
		test("should display cumulative P&L chart", async ({ page }) => {
			const chart = page.locator(
				'[data-testid="cumulative-pnl"], :has-text("Cumulative")'
			)
		...
		test("should show line chart", async ({ page }) => {
			...
			const chart = page.locator(
				".recharts-wrapper, .recharts-line, .recharts-area, .recharts-surface"
			)
			if ((await chart.count()) > 0) {
				await expect(chart.first()).toBeVisible()
			}
	...
	test.describe("Expected Value Section", () => {
		test("should display expected value section", async ({ page }) => {
			const section = page.locator(
				'[data-testid="expected-value"], :has-text("Expected Value")'
			)
		...
		test("should show average win/loss", async ({ page }) => {
			const avgWin = page.locator(
				':has-text("Avg Win"), :has-text("Média Ganho")'
			)
			const avgLoss = page.locator(
				':has-text("Avg Loss"), :has-text("Média Perda")'
			)
		...
		test("should show projected P&L for 100 trades", async ({ page }) => {
			const projection = page.locator(
				':has-text("100 trades"), :has-text("Projected")'
			)
	...
	test.describe("R-Distribution Chart", () => {
		test("should display R-distribution section", async ({ page }) => {
			const section = page.locator(
				'[data-testid="r-distribution"], :has-text("R-Distribution"), :has-text("Distribution")'
			)
			if ((await section.count()) > 0) {
				await expect(section.first()).toBeVisible()
			}
		...
		test("should show histogram bars", async ({ page }) => {
			...
			const chart = page.locator(
				".recharts-bar, .recharts-bar-rectangle, .recharts-wrapper"
			)
			if ((await chart.count()) > 0) {
				await expect(chart.first()).toBeVisible()
			}
	...
	test.describe("Tag Analysis", () => {
		test("should display tag analysis section", async ({ page }) => {
			const section = page.locator(
				'[data-testid="tag-cloud"], :has-text("Tag"), :has-text("Setup")'
			)
			if ((await section.count()) > 0) {
				await expect(section.first()).toBeVisible()
			}
	...
	test.describe("Time-Based Analysis", () => {
		test("should display time analysis section", async ({ page }) => {
			const section = page.locator(
				'[data-testid="time-analysis"], :has-text("Time Analysis")'
			)
			if ((await section.count()) > 0) {
				await expect(section.first()).toBeVisible()
			}
		...
		test("should display hourly performance chart", async ({ page }) => {
			const hourlyChart = page.locator(
				'[data-testid="hourly-chart"], :has-text("Hour")'
			)
		...
		test("should display day of week chart", async ({ page }) => {
			const dayChart = page.locator(
				'[data-testid="day-of-week-chart"], :has-text("Day of Week")'
			)
		...
		test("should display time heatmap", async ({ page }) => {
			const heatmap = page.locator(
				'[data-testid="time-heatmap"], :has-text("Heatmap")'
			)
