...
export interface StrategyMap {
	...
	greenline: Map<string, string>
	stopLossLab: Map<string, string>
	beginner: Map<string, string>
}
...
export const seedStrategies = async (
	...
	await sql`
		INSERT INTO strategies (id, account_id, name, code, description, final_r, max_risk_percent, is_active) VALUES
			(gen_random_uuid(), ${accounts.greenline.id}, 'Breakout', 'BREAKOUT', 'Trade breakouts from consolidation', 2.0, 1.0, true),
			(gen_random_uuid(), ${accounts.greenline.id}, 'Trend Following', 'TREND', 'Follow established trends', 3.0, 1.5, true),
			(gen_random_uuid(), ${accounts.greenline.id}, 'Mean Reversion', 'REVERSION', 'Fade extreme moves back to mean', 1.5, 0.75, true),
			(gen_random_uuid(), ${accounts.greenline.id}, 'Support/Resistance', 'SR', 'Trade bounces from key levels', 2.0, 1.0, true)
	`

	await sql`
		INSERT INTO strategies (id, account_id, name, code, description, final_r, max_risk_percent, is_active) VALUES
			(gen_random_uuid(), ${accounts.stopLossLab.id}, 'Aggressive Breakout', 'AGGR_BREAKOUT', 'Wider entries on breakouts', 2.0, 2.0, true),
			(gen_random_uuid(), ${accounts.stopLossLab.id}, 'Counter-Trend', 'COUNTER', 'Fade trending moves', 1.5, 1.5, true),
			(gen_random_uuid(), ${accounts.stopLossLab.id}, 'Scalping', 'SCALP', 'Quick in-and-out trades', 1.0, 1.0, true)
	`

	await sql`
		INSERT INTO strategies (id, account_id, name, code, description, final_r, max_risk_percent, is_active) VALUES
			(gen_random_uuid(), ${accounts.beginner.id}, 'Trend Following', 'TREND', 'Follow established trends', 2.0, 0.5, true),
			(gen_random_uuid(), ${accounts.beginner.id}, 'Support/Resistance', 'SR', 'Trade bounces from key levels', 2.0, 0.5, true)
	`
	...
	for (const row of rows) {
		...
		} else if (row.account_id === accounts.greenline.id) {
			map.greenline.set(row.code, row.id)
		} else if (row.account_id === accounts.stopLossLab.id) {
			map.stopLossLab.set(row.code, row.id)
		} else if (row.account_id === accounts.beginner.id) {
			map.beginner.set(row.code, row.id)
		}
	}
	return map
}
