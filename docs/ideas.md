...
- **Status (2026-06-20)**: ✅ **Phase 1 scoped → [`docs/plans/ai-assistant-phase-1.md`](plans/ai-assistant-phase-1.md)**. Four PRs filed in [`docs/backlog.md`](backlog.md) under "AI Assistant — Phase 1 (Narrator)". Phase 1 = read-only narrator on `/journal/[id]`, hard $5/user/month budget, no recommendations, no writes. The remainder of this entry is kept as the **Phase 2** reference (conversational optimizer, global drawer, RAG over docs, Hawks lab) — do NOT start before Phase 1 ships and hits its promote criteria.
- **Status (original)**: Raw brainstorm. Needs hard scoping before it becomes anything.
...
- **2026-06-17 update — boundary with the enrichment pipeline now matters**: the Two-Phase Journaling enrichment pipeline (operations CSV / candle math / indicator readout / deterministic SL-TP) is the **deterministic engine score** for a single trade. The next pass we're adding — `hawksEngineReplayPass` — replays the Hawks playbook orchestrator at the trade's entry brick and reports tier (AAA/AA/A/B/None), boosters fired, vetoes active, indicator-vs-threshold deltas. **That's the data this AI agent should _narrate_, not invent.** Phase-1 explainer scope: the agent reads `hawks_engine_replay` JSON from `trade_enrichment_snapshots` and writes coaching prose ("you took 4 SHORTs this week with macd-misaligned, 3 of them flagged would-not-fire by the engine — engine win-rate on these inputs is 14% per the catalog audit"). The agent _cannot_ compute the tier itself, _cannot_ override engine vetoes, _cannot_ propose new SL/TP — those are deterministic-engine jobs. This boundary keeps the AI value tight to _cross-trade pattern narration over engine-scored truth_, which is the moat (engine + data Axion uniquely has), and away from financial-decision hallucination.
- **Source**: 2026-06-12 session — Ygor brainstorm "integration of AI on platform, agent expert in our software and expert in trading (fed by our data and studies), help me have better insights of trades, analyze better backtesting data, optimized strategies, even create playbooks". Merged with the older "Assistant reviewer" entry (originally filed by Ygor — "AI looking at numbers, comparing with benchmarks, preview of future, giving insights") on 2026-06-12. Companion idea to the broker-integration one filed above. Boundary-with-enrichment update added 2026-06-17 after the journal/enrich pipeline session ("we have the indicators engines working, they should score the trades and enrich them").
