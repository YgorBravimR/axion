...
- **How it bit us (three times)**: (1) `src/__tests__/setup.ts` called `loadEnvFile(".env")` which itself threw ENOENT on CI — fixed by swallowing ENOENT (commit eba1dfe3). (2) `brick-size-resolver.test.ts` imported the pure `rNumberToPoints` via the barrel `@/lib/enrichment/brick-size-resolver`, which also imports `@/db/drizzle` for an unrelated DB function → `DATABASE_URL missing` on CI, taking down the suite (commit 2065d25b). (3) 2026-07-07: `real-carry-forward.test.ts` imported pure capital math from `@/lib/fractal-plan/real-carry-forward`, which also imports `@/db/drizzle` for its query function — red Unit Tests on main one day after this gotcha was logged (post-mortem: `docs/postMorten/backend.md` [BUG-2026-07-07]). All invisible locally because `.env` supplies `DATABASE_URL`. **Three occurrences = the recipe alone isn't preventing it. Durable-fix candidate: vitest globalSetup that unsets `DATABASE_URL` so local runs behave like CI.** Any agent prompt requesting tests against a DB-importing module MUST include the `vi.mock("@/db/drizzle", ...)` recipe.
...
## Vertical lines on a wide price axis: never use a 2-point LineSeries — use a series primitive

- **What**: The day/week session-boundary markers first shipped (commit `f5668464`) as a 2-point `LineSeries` from `(brickIdx, priceMin)` to `(brickIdx+1, priceMax)`, where priceMin/priceMax were the extremes of the WHOLE series. On the Hawks 5m pane the full-year price range is ~163000–188000, so a "1 brick wide, 25000 tall" segment renders as a giant DIAGONAL streak, not a vertical line. It only looks vertical when zoomed in enough that the price slope is negligible — at normal zoom it's an obvious slanted line crossing half the chart. The same footgun sits in the user-vline drawing renderer; it just went unnoticed because user vlines are rare and short-window.
- **Why it's easy to miss**: lightweight-charts has NO native vertical-line primitive, so the "2 points spanning min→max" trick is the obvious first reach and looks fine in a narrow price window / on a screenshot zoomed in. It breaks precisely when the visible price range is large relative to one brick's x-width.
- **What to do**: For true vertical lines (grid dividers, event markers), implement an LWC 5 **series primitive** (`ISeriesPrimitive`) that paints in canvas coords: `chart.timeScale().timeToCoordinate(brickIdx)` → x pixel, then `fillRect(x, 0, width, paneHeight)` inside `target.useBitmapCoordinateSpace(...)`. `zOrder(): "bottom"` puts it behind candles like grid. Single source: `src/lib/chart/vertical-lines-primitive.ts` (`VerticalLinesPrimitive`). Attach it INSIDE the chart-init effect (keyed on `[theme]`) right after `candleSeriesRef.current` is set, and re-seed its markers from a ref — an outer once-only `useEffect(…, [])` leaves a stale primitive on a destroyed series after a theme rebuild and the lines vanish.
- **Date logged**: 2026-07-14. Fix replaced `f5668464`'s LineSeries approach with the primitive; boundary detection (`computeBoundaryMarkers` / `brtIsoWeekKey`) was already correct and untouched.

---

## `fancy-canvas` types can't be imported by name — derive them from lightweight-charts

- **What**: LWC 5 primitive `draw(target, utils?)` targets are typed `CanvasRenderingTarget2D` / `DrawingUtils` from the `fancy-canvas` package. LWC re-uses them internally (`node_modules/lightweight-charts/dist/typings.d.ts` line 3: `import { CanvasRenderingTarget2D } from 'fancy-canvas'`) but does NOT re-export them from its main entry, and under pnpm `fancy-canvas@2.1.0` lives only in `node_modules/.pnpm/…` — it is NOT hoisted to top-level `node_modules/fancy-canvas`. So `import type { CanvasRenderingTarget2D } from "fancy-canvas"` passes strict ESLint but fails `tsc` with `TS2307: Cannot find module 'fancy-canvas'`, and the untyped callback param then trips `TS7006 implicit any`.
- **Why it's easy to miss**: strict-eslint (`eslint.config.strict.mjs`) resolved the import fine; only a real `tsc --noEmit` surfaced it. An agent that only runs `pnpm lint` / `pnpm lint:strict` (which is eslint + i18n, NOT tsc) reports green while the prod build would break.
- **What to do**: Don't import from `fancy-canvas`. Derive the target type from LWC's own contract: `type DrawTarget = Parameters<IPrimitivePaneRenderer["draw"]>[0]`. Zero extra import, resolves through the already-installed lightweight-charts. And remember: `pnpm lint:strict` does NOT typecheck — run `npx tsc --noEmit -p tsconfig.json` for any change touching third-party type surfaces.
- **Date logged**: 2026-07-14. File: `src/lib/chart/vertical-lines-primitive.ts`.

---

## Next 16.3 moved the `◐` build marker from the parent route to its locale children

- **What**: After `next` 16.2.9 → 16.3.5, `pnpm build` prints every page route's group row with a BLANK render-mode marker, and puts `◐` on the three locale child rows instead. Diffing the parent rows across the two versions shows all 37 routes going `◐` → nothing, which reads exactly like "Partial Prerendering was silently disabled by the upgrade" — on an auto-deploying `main` that looks like a prod perf regression worth reverting for. It isn't. The `◐` count goes 37 → 111 (37 routes × 3 locales: `/[locale]`, `/pt-BR`, `/en`). PPR is fully intact; only the display row moved.
- **Why it's easy to miss**: the legend at the bottom of the build output is unchanged across both versions, so the symbols mean the same thing — you have to compare the INDENTED child rows, not the `├ ` group rows, to see it. `grep -E "^├ "` (the natural way to extract the route table) drops exactly the rows that still carry the marker.
- **What to do**: to compare prerender modes across a Next upgrade, count markers (`grep -c '◐'`) rather than diffing route rows, and inspect a full route block (parent + children) before concluding anything changed. Expect `routes × locales` for a locale-segmented app, not `routes`.
- **Also new in 16.3**: a dev-only insight now fires on EVERY route — `Route "/[locale]/…": Next.js encountered uncached data during prerendering or a navigation`. It traces to `requireAuth()` → `auth()` in `src/app/actions/auth.ts`, called outside `<Suspense>`. Zero occurrences on 16.2.9, one per route on 16.3.5. It does NOT change the production build (routes still PPR) — it is a diagnostic, not a regression. Fixing it properly means wrapping the auth-dependent subtree in `<Suspense>` or opting the route out with `export const instant = false`.
- **Date logged**: 2026-09-15. Found while upgrading 48 non-major deps (commit `9fd92c66`).

---

## `.vercelignore` and `outputFileTracingIncludes` collide silently — the deploy dies at upload, not at build

- **What**: `data/` sat in `.vercelignore` (commit `e783e1ee`) while four server routes read `data/hawks/user-entries` through `process.cwd()` (commit `7254a9d3`). Next traces those runtime reads into the output file manifest, Vercel then `lstat`s a path that was never uploaded, and `vercel deploy --prebuilt` exits 1 with `Error: ENOENT: no such file or directory, lstat '/vercel/path0/data/hawks/user-entries/2026-03-02.json'`. Both commits are individually correct. Neither one broke anything on its own.
- **Why it's easy to miss**: `pnpm build` and `vercel build` both pass, because the files exist in the checkout. The failure only appears in the _deploy_ step, after the upload, in a job most agents read as "the build broke". `main` auto-deploys with no other gate, so production silently stopped shipping on 2026-08-18 and nobody noticed for a month — six merged commits sat on `main` and were not live.
- **What to do**: before adding a directory to `.vercelignore`, run `grep -rn "process.cwd()" src/` and confirm no runtime code reads under it. If something does, ship the files rather than excluding the path from tracing — excluding makes the routes throw ENOENT and return 500 in production, which is a silent break. Here the whole directory is 236K across 59 files, so shipping it costs nothing next to the 67MB `libduckdb.so` the same config already traces in on purpose. Verify with `gh run list --workflow="Personal Vercel Deploy" --branch main --limit 3 --json conclusion,headSha`, never by assuming a merge to `main` reached production.
- **Date logged**: 2026-09-16. Fix narrowed the `.vercelignore` rule to `data/hawks/candles/` + `data/parquet/` (both gitignored, so neither is in the CI checkout anyway).
