...
### Hawks `EntryQualityGates.keltner*` toggles are dead UI — config exists, engine never reads

- **What**: `qualityGates.keltnerOuterBlock`, `keltnerInnerPenalty`, `keltnerNearBricks`, `keltnerInner.mode` all exist as `EntryQualityGates` fields with full UI controls in `hawks-quality-controls.tsx:280-308`, persisted in the run-config schema, surfaced in the run-builder UI. **None of them control engine behaviour.** A `grep -rE 'kc1_inf|kc1_sup|kc2_inf|kc2_sup' src/lib/backtest` returns 0 matches — the engine has zero Keltner reads. Flipping any of these toggles on a run leaves the fire count, exit modes, and PnL identical. Group C indicator-isolation audit on 2026-06-15 confirmed and graded this NOT WIRED. Only `keltnerSignalAt` in `hawks-isolation-charts.tsx:834` reads the columns, and only for visual lab badges (not engine state).
- **What to do**: Do **not** rely on these toggles when reasoning about a run's fire pattern — they're noise in the leaves catalog. When the wiring lands (`walkKeltner` + `readKeltner`, planned per `docs/hawks-strategy/indicator-isolation/group-c-keltner.md`), it should consume these fields OR delete them; the half-state is the trap. If you see a backlog item suggesting "tune `keltnerInnerPenalty`", flag that it does nothing today.
- **Source**: `src/types/backtest.ts:197-274`, `src/lib/backtest/presets/hawks-presets.ts`, audit at `docs/hawks-strategy/indicator-isolation/group-c-keltner.md`, script at `scripts/indicator-isolation/group-c-keltner.ts`.
- **Date logged**: 2026-06-15.
