...
| A5–A16 | high/medium/low | mixed      | sidebar / login-form / account-switcher / user-menu | tooltips on collapsed nav, indent flatten, dialog viewport guard, login lg widen, aria-label expanded variant, copyright DRY, icon `aria-hidden`, logo transition stacking | **deferred** (post-incident; not responsive-blockers — see Backlog) |
...
- **Cluster A polish** (12 findings A5–A16): sidebar tablet tooltips, indent flatten, dialog viewport guard on account-switcher, login lg:max-w, aria-label on expanded user-menu trigger, copyright DRY extraction, icon `aria-hidden`, font-scale drift in account-switcher, logo transition stacking, sidebar memoization. None are responsive-blockers; lost in the mid-scan state incident (see Incident below).
